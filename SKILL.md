...
Use this project skill when the user explicitly invokes `/unreal-mcp-skills` or `$unreal-mcp-skills`, asks to control or inspect the currently running UE editor through MCP, asks how to enable Unreal MCP, or asks which UE MCP Toolset should handle an editor task.
...
The tracked source lives at `skills/unreal-mcp-skills/`. The installed runtime copy currently lives at `C:\Users\cbsjz\.cc-switch\skills\unreal-mcp-skills\`. Keep both copies synchronized after edits.
...
- Agent skill activation: the external agent loads this file when `$unreal-mcp-skills` or the skill name is used.
